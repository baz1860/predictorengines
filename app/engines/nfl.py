"""NFL engine adapter (mirrors app/engines/cfb.py and app/engines/nhl.py)."""
...
ENGINE_DIR = ROOT / "nfl"
GAMES_CSV = ENGINE_DIR / "data" / "games.csv"


class NFLAdapter(EngineAdapter):
    id = "nfl"
    name = "NFL"
    sport = "nfl"
    ...
    def _run(self, command: str, params: dict | None = None):
        from nfl import engine as nfl_engine
        return run_inprocess(nfl_engine.COMMANDS, command, params)
    ...
    def edge_schema(self) -> dict[str, Any]:
        return {
            ...
            "odds_sources": [{"id": "manual", "label": "Manual nfl/odds.csv"}],
    ...
    # ── settlement (pure pandas; no nfl import) ──────────────────────────────
    def grade_open_bets(self, rows: pd.DataFrame) -> dict[int, tuple]:
        ...
        for i, r in rows.iterrows():
            g = games[(games["home"] == r.get("home")) & (games["away"] == r.get("away"))
            ...
            status = self._grade_row(r, margin, total)
            if status:
                out[i] = (status, f"{int(g['home_score'])}-{int(g['away_score'])}")
        return out

    @classmethod
    def _grade_row(cls, r, margin: int, total: int) -> str | None:
        ...
        if market == "spread":
            if line is None:
                return None
            # traditional convention (negative = home favorite), matching
            # edge.py's odds.csv and bankroll.py's settle_bet.
